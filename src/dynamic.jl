...
storage(t::Task) = t.storage == nothing ? (t.storage = ObjectIdDict()) :
                   t.storage

isroot(task::Task) = task.parent == task

bindings(task::Task) = get!(storage(task), :bindings, @d())
...
function binding(b::Binding{T}, t::Task = current_task()) where {T}
    haskey(bindings(t), b) ? bindings(t)[b]::T :
    isroot(t) ? root(b) : binding(b, parent(t))
end

function set!(b::Binding{T}, v::T, t::Task = current_task()) where {T}
